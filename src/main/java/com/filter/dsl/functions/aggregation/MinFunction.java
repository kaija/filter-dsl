...
/**
 * MIN function - Returns the minimum value from a collection of comparable values.
 * 
 * Usage: MIN(collection)
 * 
 * Examples:
 * - MIN([1, 2, 3, 4, 5]) -> 1
 * - MIN([5.5, 2.2, 8.8, 1.1]) -> 1.1
 ...
 */
public class MinFunction extends DSLFunction {
    ...
    public FunctionMetadata getFunctionMetadata() {
        return FunctionMetadata.builder()
            ...
            .description("Returns the minimum value. Defaults to userData.events with optional filter condition.")
            .build();
    }

    @Override
    public AviatorObject call(Map<String, Object> env, AviatorObject... args) {
        // No arguments: MIN() -> min from all events
        if (args.length == 0) {
            Collection<?> events = getUserDataEvents(env);
            if (events.isEmpty()) {
                return AviatorNil.NIL;
            }
            return minCollection(events);
        }

        Object firstArg = getValue(args[0], env);

        // Single string argument: MIN("condition") -> filter and find min
        if (args.length == 1 && firstArg instanceof String) {
            ...
            return minCollection(filtered);
        }

        // Single collection argument: MIN(collection) -> legacy syntax
        if (args.length == 1) {
            if (firstArg == null) {
                return AviatorNil.NIL;
            }
            ...
            if (firstArg instanceof Collection) {
                ...
                return minCollection(collection);
            }

            if (firstArg.getClass().isArray()) {
                return minArray(firstArg);
            }

            throw new com.filter.dsl.functions.TypeMismatchException(
                "MIN expects a collection, array, or filter condition, got " + firstArg.getClass().getSimpleName()
            );
        }

        // Two arguments: MIN(collection, "condition") -> legacy syntax with filter
        if (args.length == 2) {
            Object conditionObj = getValue(args[1], env);
            if (!(conditionObj instanceof String)) {
                throw new com.filter.dsl.functions.TypeMismatchException(
                    "MIN condition must be a string expression"
                );
            }
        ...
        throw new com.filter.dsl.functions.FunctionArgumentException(
            "MIN expects 0-2 arguments, got " + args.length
        );
    }
     ...
     * 
     * @param collection The collection to find minimum from
     * @return AviatorObject containing the minimum value, or null if no valid values
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    private AviatorObject minCollection(Collection<?> collection) {
        ...
        for (Object item : collection) {
            ...
            if (item instanceof com.filter.dsl.models.Event) {
                ...
                if (params != null) {
                    // Find min among all numeric parameters from the event
                    for (Object paramValue : params.values()) {
                        if (paramValue instanceof Number) {
                            if (minValue == null) {
                                ...
                                int comparison = compareNumbers((Number) paramValue, (Number) minValue);
                                if (comparison < 0) {
                                    minValue = paramValue;
                                    if (paramValue instanceof Double || paramValue instanceof Float) {
                                        hasDouble = true;
            ...
            if (minValue == null) {
                ...
                if (!(item instanceof Comparable)) {
                    throw new com.filter.dsl.functions.TypeMismatchException(
                        "MIN expects comparable values, got " + item.getClass().getSimpleName()
                    );
                }
                
                if (!(minValue instanceof Comparable)) {
                    throw new com.filter.dsl.functions.TypeMismatchException(
                        "MIN expects comparable values, got " + minValue.getClass().getSimpleName()
                    );
                }
                
                // Check if types are compatible for comparison
                if (!areComparable(minValue, item)) {
                    throw new com.filter.dsl.functions.TypeMismatchException(
                        "MIN cannot compare " + minValue.getClass().getSimpleName() + 
                ...
                try {
                    int comparison;
                    if (item instanceof Number && minValue instanceof Number) {
                        comparison = compareNumbers((Number) item, (Number) minValue);
                    } else {
                        comparison = ((Comparable) item).compareTo(minValue);
                    }
        ...
        return wrapValue(minValue, hasDouble);
    }
    
    /**
     * Find the minimum value in an array.
     * 
     * @param array The array to find minimum from
     * @return AviatorObject containing the minimum value, or null if no valid values
     */
    @SuppressWarnings({"unchecked", "rawtypes"})
    private AviatorObject minArray(Object array) {
    ...
    private AviatorObject wrapValue(Object value, boolean hasDouble) {
        if (value instanceof String) {
            ...
            return new AviatorJavaType("minValue") {
                @Override
